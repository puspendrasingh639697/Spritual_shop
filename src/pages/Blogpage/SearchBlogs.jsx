...
import { Sparkles, Gem, Home, BookOpen, ArrowRight, Search } from "lucide-react";
...
const SearchBlogs = () => {
  ...
  const filteredBlogs = staticBlogs.filter((blog) => {
    const matchesCategory = selectedCategory === "all" || blog.category.name.toLowerCase().includes(selectedCategory.toLowerCase());
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              type="text"
              placeholder="Search spiritual blogs..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-4 py-2 bg-white border border-[#edd5b9] rounded-full text-xs text-[#4a2e18] focus:outline-none focus:ring-2 focus:ring-[#8b3a2b] shadow-sm"
            />
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBlogs.length > 0 ? (
            ...
            <div className="col-span-full text-center py-12">
              <p className="text-stone-600 font-serif text-base">No matching blogs found for your search.</p>
            </div>
          )}
        </div>
