...
const RelatedProducts = ({ currentProductId }) => {
  const navigate = useNavigate();
  const { fetchProducts, getRelatedProducts } = useProductStore();

  useEffect(() => {
    fetchProducts();
  }, [fetchProducts]);

  // 🔗 Direct store se related products uthao
  const relatedList = getRelatedProducts(currentProductId, 4);
          ...
          {relatedList.map((product) => {
            ...
            return (
              ...
              >
                <div>
                  <div className="w-full h-48 bg-white border border-stone-200 rounded-sm mb-3 overflow-hidden flex items-center justify-center relative">
                    {/* 🛡️ Empty string warning fix: Render image only if prodImage exists */}
